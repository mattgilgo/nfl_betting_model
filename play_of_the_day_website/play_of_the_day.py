...
def play_of_the_day(print_play_of_the_day=True):
    weekly_game_info_df = choose_bets.choose_bets(print_picks=False, input=16)
    ...
    if (best_spread_pick >= 30) or (best_over_under_pick >= 30):
        ...
        best_pick_val = "No Locks this week."
        confidence = 0
        if print_play_of_the_day:
            print("No Locks for this week's games")
        return best_pick_val, confidence
